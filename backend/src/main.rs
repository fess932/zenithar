...
async fn main() -> Result<()> {
    ...
    let app = Router::new()
        ...
        // Sticker/emoji/gif packs — grouped saved_items, shared by slug.
        .route("/api/packs", get(packs::list).post(packs::create))
        .route("/api/packs/of/{id}", get(packs::list_of))
        .route(
            "/api/packs/import",
            post(packs::import).layer(DefaultBodyLimit::max(uploads::MAX_VIDEO_BYTES + 1024)),
        )
        .route(
            "/api/packs/{id}",
            patch(packs::update).delete(packs::delete),
        )
        .route(
            "/api/packs/{id}/items",
            post(packs::add_items).layer(DefaultBodyLimit::max(uploads::MAX_VIDEO_BYTES + 1024)),
        )
        .route(
        ...
        .route(
            "/api/packs/{id}/items/{item_id}/file",
            get(packs::serve_shared_item),
        )
        .route("/api/packs/{id}/add", post(packs::add_from_slug))
    ...
    info!(
        ...
        commit_msg = env!("ZENITHAR_GIT_MSG"),
        "zenithar backend listening on http://{bind}"
    );
